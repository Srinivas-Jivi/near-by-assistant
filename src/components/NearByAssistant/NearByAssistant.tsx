...
export const NearByAssistant: React.FC<NearByAssistantProps> = ({ 
  ...
  }, [activeCategory, currentCategory, hasSpecialties, userLocation, locationLoading, searchLoading, searchError, mergedConfig.searchConfig?.enableSpecialtyFilter]);
...
const NearbyPlacesApp: React.FC = () => {
  const config: NearByAssistantConfig = {
    // Use the demo search service for testing
    searchService: createDemoSearchService(),
    
    // Or implement your own search service
    // searchService: {
    ...
    //     return `https://your-api.com/photos/${photoRef}`;
    //   },
    ...
    navigation: {
      onPlaceClick: (place, categoryType) => {
        console.log('Place clicked:', place, categoryType);
        // Navigate to place details
        window.location.href = `/place/${place.place_id}`;
      },
      onCallClick: (phoneNumber, place) => {
      ...
      onBookingClick: (place, categoryType) => {
        console.log('Booking clicked:', place);
        // Handle booking logic
        window.location.href = `/book/${place.place_id}`;
      },
    },
